...
\begin{frame}
  ...
  \begin{block}{Kinds of books (existing)}
    \TODO{}{}
  \end{block}
\end{frame}
...
\begin{frame}
  \frametitle{Example-driven books}
  \begin{block}{}
    \begin{itemize}
...
\begin{frame}
  \begin{block}{}
    \begin{itemize}
    \item Sequences associated with hydra battles are studied
...
\begin{frame}
  ...
  \begin{block}{}
    Ordinals less than $\epsilon_0$ are represented as \emph{ordinal notations}: comparison, canonical sequences, partition limit/successors/zero ordinals are implemented by certified functions, allowing the user to write proofs by computation.
...
\begin{frame}
  \begin{block}{}
    The functions $H'_\alpha$ are ``implemented'' with \textcolor{plugincolor}{coq-equations}.
    \vspace{5pt}
    \begin{footnotesize}
  ...
  \begin{block}{}
    Using combinatorial lemmas by  [K. \& S.], we prove (by transfinite induction on \textcolor{mathcolor}{$\alpha$}):
    \begin{quote}
      \begin{itemize}
...
\begin{frame}
  ...
  \begin{block}{}
    \begin{itemize}
    \item Changes in definitions and proofs already happen
      (\emph{e.g.} type class structure), without making the book's maintenance difficult.
