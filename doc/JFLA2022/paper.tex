...
\begin{figure}[h]
 ...
 \inputsnippets{Hprime_HprimeDef}     
    \end{minipage}}
 \caption{$H'$ definition with the \equations
...
%
The original release of the Cantor library preceded support for type classes in \coq~\cite{MS08}. Changes over time before the move of \Hydras to \community have since then introduced type classes for some concepts in the Hydra-battles library related to ordinals. Most type classes initially used a \emph{semi bundled} approach~\cite{Mathlib}, but we have recently \emph{unbundled} classes to enable more sharing during resolution~\cite{BS2011}. For example, we use the (single-field) \texttt{Decision} type class~\cite{BS2011}, that describes deciding a proposition, in parameters to other classes rather than in fields. Due to the modest depth of our class hierarchy, we believe unbundling will not lead to scalability issues. Orthogonally to type classes, we introduced automation for definitions via the \equations~\cite{sozeau:hal-01671777} and Paramcoq~\cite{Keller2012} plug-ins.
...
The flip side of our approach is that we separate the Coq sources from the LaTeX sources that describe the source code, and our use of snippet delimiters in the Coq source code files may also lower source readability. For books and libraries with small scope and specialized audiences, distributing knowledge representations between sources and documentation formats the way we do in \Hydras may introduce unnecessary overhead without providing additional benefits.
%
