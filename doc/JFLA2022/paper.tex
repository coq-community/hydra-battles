...
Hydra games (also know as \emph{Hydra battles}) appear in an article published in 1982 by two mathematicians, 
Laurie Kirby and Jeff Paris~\cite{KP82}: \emph{Accessible Independence Results for Peano Arithmetic}.
This article describes a game between two players: Hercules and a hydra, and 
...
Indeed, like Goodstein sequences~\cite{goodstein_1944, Sladek07, KP82}, hydra battles are just 
 a gameful presentation of accessibility problems in the set of ordinals below $\epsilon_0$. 
...
Whenever possible, we try to present several implementations of the same object, several proofs of the same statement.
Let us take an example. Ordinal numbers (strictly below $\Gamma_0$) are represented with the help of \emph{ordinal notations}, \emph{i.e.} data types:
\begin{itemize}
\item natural numbers (ordinals below $\omega$),
\item list of natural numbers (ordinals below $\omega^\omega$),
\item ternary trees (ordinals below $\epsilon_0$ in Cantor normal form),
\item quaternary trees (ordinals below $\Gamma_0$ in Veblen normal form).
\end{itemize}
Comparison and arithmetic operations are implemented by
structurally recursive functions.
Figure~\ref{fig:Ex42E0} shows a trivial (computational) proof of the equality \ref{eq:1}.
...
By definition, $\omega^2=\phi_0(2)$ is additive principal, thus
satisfies the equality. Figure~\ref{fig:Ex42b} shows the mians steps of the proof.
