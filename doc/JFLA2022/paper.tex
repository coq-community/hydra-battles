...
In 2020, members of \community transferred the Gaia source code to GitHub and adapted it for recent releases of the Mathematical Components library, which Gaia heavily relies on. A volunteer finished the only in-progress proof left by Grimm. At around 165,000 lines of code, Gaia is currently one of the largest maintained open source Coq projects~\cite{Gaia}.
...
\node[rectangle, draw=black,minimum height=0.5cm,minimum width=1.5cm] (cantor) { Cantor };
\node[rectangle, draw=black,minimum height=0.5cm,minimum width=1.5cm,right of=cantor, node distance=3cm] (additions) { Additions };
\node[rectangle, draw=black,minimum height=0.5cm,minimum width=1.5cm,right of=additions, node distance=3cm] (additions) { Goedel };
...
This build system, originally written for OCaml, but that was extended to support Coq projects, allows building packages contained in a single source tree separately, which is essential to be able to publish the different libraries contained in a single repository as separate (opam) packages to the Coq package index.
...
\label{sect:acks}
We are grateful to the original authors and current maintainers of the Coq packages we use and depend on: José Grimm (Gaia), Russel O'Connor(Goedel), Matthieu Sozeau (Equations), the Mathematical Components team (ssreflect, algebra), Marc Lasson and Chantal Keller (Paramcoq), and the authors and maintainers
of \coq and its associated tools.
