...
%
This umbrella project now includes evolved versions of the former Cantor~\cite{CantorContrib} and Additions~\cite{AdditionsContrib} libraries (under the new names of Hydra-battles and Addition-chains), the Ackermann sub-library which was extracted from Russel O'Connor's Goedel library~\cite{OConnor05, Goedel}, and a bridge to the \gaia library by José Grimm~\cite{Gaia,grimm:hal-00911710}.
%
...
We chose to build our library and book on two simple themes which allow many variations: computing powers in a monoid, and Kirby and Paris' hydra battles, formerly user contributions of the Coq system. In the interest of space, we shall present only the second one in this article.
