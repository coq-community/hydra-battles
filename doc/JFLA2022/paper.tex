...
\newcommand{\coqmakefile}{coq\_makefile\xspace}
...
In practice, we still support building the whole project using \make via the \coqmakefile tool bundled with \coq, and we actually rely on this build system in our documentation generation pipeline. The main reason is that using \coqmakefile is currently more convenient than Dune for local IDE-based Coq code development, e.g., due to Dune placing generated files in a separate directory. We expect to migrate fully to Dune in the near future.
...
It is also a medium sized library (more than 50,000 LOC), dependent on various tools and libraries of the \coq ecosystem, which may be also used to experiment with new maintenance techniques of the code and its documentation.
