...
\begin{abstract}
  \Hydras is a collaborative library of discrete mathematics,
  written for the \coq proof assistant and developed as part
  of the \community project on GitHub. The Coq code is accompanied
  ...
  inside and outside \community) and its reverse dependencies (\community
  projects that depend on it). Second, the book needs to be consistent
  with the library Coq code, which undergoes frequent changes to improve
  structure and include new definitions and results.
\end{abstract}
...
For example, the core projects in the Mathematical Components family for Coq amount to more than 160,000 lines of code and 11,000 lemmas~\cite{Nie2020a}, while Lean's Mathlib consists of more than 140,000 lines of code and 34,000 declarations~\cite{Mathlib}. However, after a key mathematical definition or result is added to a library, it needs be \emph{documented} and \emph{maintained}~\cite{vanDoorn2020}.
...
While books that document proof assistant libraries are valuable to beginners, the purpose of current books is mainly instrumental, i.e., to teach a certain topic or technique. We believe that such books can become \emph{ends in themselves}---not just sources of learning, but also sources of inspiration and entertainment.
