...
\begin{abstract}
  ...
  We present its evolution since a former presentation~\cite{JFLA2018paper} and how its structure is determined by other libraies and the \emph{Alectryon} literate proving tool.
\end{abstract}
...
\item At every round, Hercules chops off one head of the hydra. If the head is at a distance greater or equal than 2 from the root,
  ...
  Figure~\ref{fig:round} shows an example of such a transformation with $n=2$.
\end{itemize}
