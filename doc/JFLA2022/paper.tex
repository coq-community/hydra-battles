...
\begin{figure}[h]
 ...
 \inputsnippets{Hprime_HprimeDef}     
    \end{minipage}}
 \caption{$H'$ definition using the \equations plug-in.}
\label{fig:Hprime}
\end{figure}
...
\paragraph{Proof engineering improvements.}
%
The original release of the Cantor library preceded support for type classes in \coq~\cite{MS08}.
%
Changes over time before the move of \Hydras to \community have since then introduced type classes for some concepts in the Hydra-battles library related to ordinals, e.g., to provide a uniform way to compare ordinals within different representation systems.
%
Most type classes initially used a \emph{semi bundled} approach~\cite{Mathlib}, but we have recently \emph{unbundled} classes to enable more sharing during resolution. For instance, we use the (single-field) \texttt{Decision} type class~\cite{BS2011}, that describes deciding a proposition, in parameters to other classes rather than in their fields. Due to the modest depth of our class hierarchy, we believe unbundling will not lead to scalability issues. Orthogonally to type classes, we introduced automation for definitions using the Paramcoq~\cite{Keller2012} and \equations~\cite{sozeau:hal-01671777} plug-ins, as illustrated in Figure~\ref{fig:Hprime}.
...
\end{itemize}
Table~\ref{tbl:books} shows a category breakdown for what we believe are the most prominent Coq books in English. Note that several traditional books, such as Coq'Art, used custom tools during their writing process to keep code snippets synchronized with Coq. However, once a book is published, the accompanying code generally takes on a life of its own~\cite{CoqArtCommunity} and may come to substantially diverge from the book.
...
%
Moreover, \Hydras is not limited to the study of ordinal numbers and their applications.
