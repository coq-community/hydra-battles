...
\subsection{\Hydras Genealogy, Dependencies, and Organization}
...
\node[rectangle, draw=black,minimum height=0.5cm,minimum width=1.5cm] (equations) { Equations };
\node[rectangle, draw=black,minimum height=0.5cm,minimum width=1.5cm,right of=equations, node distance=2cm] (mathcomp) { MathComp };
\node[rectangle, draw=black,minimum height=0.5cm,minimum width=1.5cm,node distance=2cm,left of=equations, node distance=4.7cm] (paramcoq) { Paramcoq };
\end{scope}
...
\draw[->,dotted] (cantor) -- (hydras.north) ;
...
\draw[->] (equations.south) -- (chains) ;
\draw[->] (paramcoq.south) -- (hydras) ;
\end{tikzpicture}
}
...
\begin{itemize}
\item \emph{Executable books} that are in effect large, well-commented Coq programs from which other representations of the material (HTML, PDF) are derived using tools.
\item \emph{Traditional books} generated from documentation languages such as LaTeX that are accompanied by standalone Coq formalizations.
\end{itemize}
