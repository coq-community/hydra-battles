...
\item At every round, Hercules chops off one head of the hydra. If the head is at a distance greater or equal than 2 from the root,
  then some sub-tree $h$ of the hydra is copied a certain amount $n$ of times. The number $n$ of copies and the copied sub-hydra $h$ may depend of the considered variant of the game
...
We present complete proof scripts for the \textit{Coq} proof assistant~\cite{Coq}. These scripts may consulted and replayed by the reader.
...
Our development may be used as a library on ordinal numbers, for instance for proving termination properties.
...
The 2018 article contains a formal proof of  a variant of Theorem~\ref{kp:thm2}:
...
consider battles where the hydra can make an arbitrary number of copies at any time, which make easier our proof by absurd.
...
Without loss of generality, we could consider the following restrictions:
 ...
 \begin{itemize}
   \item The game starts at an initial step $i\in\mathbb{N}$ (not necessariliy $0$).
...
\begin{todo}
  ...
  \item Presentation of the main steps of proofs, possibly interleaved with text (Figures \ref{fig:Ex42E0} and \ref{fig:Ex42-schutte}).
