...
The best known of total function which is not primitive recursive is the Ackermann function. We show how to adapt the classic proof (see for instance~\cite{planetmath}) to the constraints of \gallina.
...
$f\,x\,y \leq \texttt{Ack}\,n\,(\textrm{max}\,x\,y)$. Let us say that $f$ is \emph{majorized} [by some \texttt{Ack\,$n$}].
\item Show that \texttt{Ack} fails to satisfy this property.
\end{enumerate}

The second part is the easiest one, let us look at it first, and admit the following lemma.
...
Then, assume texttt{Ack} is majorized, and find a contradiction.
...
Section Impossibility_Proof.
  ...
  Fact Ack_majorized : 
      exists (n:nat), forall x y,  Ack x y <= Ack n (x + y).
...
Now it remains to prove Lemma \texttt{PR\_2\_majorized}.
...
Thus the lemma we will have to prove is, for any primitive recursive function of abritrary arity $n$ is:

\[
   \exists q\in\mathbb{N},\, \forall x_1,\dots,x_n, 
