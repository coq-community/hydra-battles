...
\emph{Primitive recursive functions} are total functions from $\mathbb{N}^n$ to
...
The four examples above are defined as \coq terms in Section~\vref{sect:primrec-examples2}
...
Let us consider again the functions defined in 
previous sections. 
...
\subsubsection{A few proofs of correctness}

It is tempting to \emph{prove} that our functions \texttt{cst0},
\texttt{plus}, \texttt{mult} and \texttt{fact} are correct implementations in \texttt{PrimRec} of the mathematical functions we
consider.
