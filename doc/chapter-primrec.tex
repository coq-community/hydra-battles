...
Before playing with primitive recursive functions in \coq{}, let us get familiar with the definition above, with the help of a few simple examples.
\label{sect:primrec-examples1}

\subsubsection{Projections}

For instance, the projection $\pi{2,3}$ satisfies the equation
$\pi{1,3}(x,y,z)=y$ for any natural numbers $x$, $y$ and $z$.
...
\subsection{Addition on natural natural numbers}
...
\subsection{Multiplication on natural natural numbers}
...
\begin{exercise}
 Show that the factorial function is primitive recursive (by the same method as above). 
\end{exercise}

\begin{remark}

The four examples above are defined in \coq in Section~\vref{sect:primrec-examples2}
...
\subsection{A few arithmetic functions}
\label{sect:primrec-examples2}
Let us show how the few functions presented in Section~\vref{sect:primrec-examples2} can be defined in \coq.

\subsubsection{Constant function of value 0}

\inputsnippets{PrimRecExamples/cst0}

\subsubsection{Addition}

Let us recall that projections $\pi{i,n}$ are represented by a 
construction \texttt{projFunc $n$ $j$ $H$} where $j=n-i$
and $H$ is a proof of $j<n$. Such equalities are registered as local remarks.



\inputsnippets{PrimRecExamples/addition}



\subsubsection{Multiplication}

\inputsnippets{PrimRecExamples/multiplication}

\subsubsection{The factorial function}

\inputsnippets{PrimRecExamples/factorial}
...
\input{movies/snippets/primRec/isPRExtEqual}
