...
We may want to prove properties shared with all the set of
primitive function, for instance that any primitive recursive function is representable by a predicate in Peano Arithmetics (see Chap~›\vref{chap:PRRep}).
In this chapter, we will prove by induction that every primitive recursive function has a given property which is not satisfied by  Ackermann function, and deduce that this function is not PR.
...
First, let us define Ackermann function in \coq, then look at a few properties of this function.
