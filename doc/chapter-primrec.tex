...
This function is definied by mutual recursion,  together with the  function 
...
\href{https://github.com/coq-community/goedel}{goedel library} fr instance), we plan to make all theses lemmas transparent.
\end{remark}
...
\section{Ackermann: a non  primitive recursive function}
...
The module \href{../theories/html/hydras.Hydra.Hydra_Theorems.html}{Hydra\_Theorems} contains a proof that the function which computes the lenght of standard hydra battles is not primitive recursive. This proof is  a little more complex than the preceding one.
...
Now, let us recall the following lemmas :
...
\begin{Coqsrc}
H_F : forall (alpha : E0) (n : nat), 
     F_ alpha (S n) <= H_ (Phi0 alpha) (S n) 
\end{Coqsrc}

\emph{Please note that our proof of this lemma is not trivial at all. It uses some lemmas of Chapters~\vref{chap:ketonen} and \vref{chap:alpha-large}. Mainly:}

\begin{Coqanswer}
H'_restricted_mono_l:
  forall (alpha beta : E0) (n : nat),
  Canon_plus n alpha beta -> H'_ beta n <= H'_ alpha n
\end{Coqanswer}

This lemma is proven in 
\href{../theories/html/hydras.Epsilon0.Hprime.html}{Epsilon0.Hprime}, with the help of the \emph{Ketonen-Solovay machinery}.
...
\begin{Coqsrc}
H'_L_: forall (alpha : E0) (i : nat), 
      H'_ alpha i <= L_ alpha (S i)
\end{Coqsrc}
