...
%
%   Note to move to Euclidean chains 
%
% \textbf{Note:}  It may be interesting to adapt the old contrib \texttt{coq-additions}, where addition-chains are represented as sequences of instructions of a small abstract machine. This work has  started in file \href{../theories/html/hydras.additions.AM.html}{\texttt{AM.v}}.
...
%  Definition stack := list M.
%  Definition config := (M * list M)%type.

%  Fixpoint exec (c : code) (x:M) (s: stack) : option config :=
%    match c, s with
%      nil, _ => Some (x,s)
