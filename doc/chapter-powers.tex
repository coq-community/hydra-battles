...
An \emph{addition chain} (In short : \emph{a chain})~\cite{brauer1939, additionchainspage} is a representation of a sequence of
...
In Sect.~\ref{sect:test-strat}, we compare the three strategies with respect to the lenght of the built chains.
...
The following function is parameterized by a startegy $\gamma$.
...
Please define an Euclidean chain generator using this data structure, and prove its correctness (done  in ~\href{../theories/html/hydras.additions.AM.html}{additions.AM}, but with clumsy fresh proofs). The project is now to simplify the proofs. 
perhaps considering chains of type \texttt{code} as a refinement of functional chains. Please note that many proof scripts are copied verbatim from 
\texttt{Euclidean\_Chains} into \texttt{AM}.
\end{project}
