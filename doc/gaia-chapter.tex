...
The function \texttt{g2h} is also used to provide a ``pretty'' printing of Cantor normal forms (through \texttt{Epsilon0.T1.pp}).
...
\section{Importing Material from \Hydras}
...
Section~\ref{sect:gaia2hydra} shows how lemmas on arithmetic properties of ordinals strictly less than $\epsilon_0$ are  imported from \gaia , with \HydrasLib's notations.
...
The library \href{../theories/html/gaia.GPaths.html}%
{\texttt{gaia.GPaths}} imports definitions and lemmas from
...
First, we import some definitions. When we import some constant \emph{Id} from {\texttt{hydras.Epsilon0.Paths}} the name
\emph{Id} is given to {\texttt{gaia.GPaths}}. In order to get
{\texttt{hydras.Epsilon0.Paths}}'s sense, please refer it as
\emph{Paths.Id}.
Sometimes, we rename \emph{Paths.Id} into \emph{{\bf h}Id}.
