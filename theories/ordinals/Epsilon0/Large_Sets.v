...
Compute L_test T1omega (fun i => S (2 * i))%nat 23.
...
Proof.
  specialize (L_lim_ok T1omega nf_omega refl_equal L_fin
                       (fun i => L_fin_ok (S i))).
  intros.
  inversion H; auto.
...
Proof.
  ...
  -   path_decompose (S k).
      ...
      + simpl (T1omega * S i). replace (i + 0)%nat with i.
...
(* begin snippet LOmegaSquareEqn *)

Lemma L_omega_square_eqn k :
  (0 < k)%nat ->
  L_omega_square k = (exp2 k * (k + 2) - 1)%nat. (* .no-out *)
(*| .. coq:: none |*)
Proof.
...
(* begin snippet LOmegaCube3Eq *)

Lemma L_omega_cube_3_eq:
  let N := exp2 95 in
  let P := (N * 97 - 1)%nat in
  L_omega_cube 3  = (exp2 P * (P + 2) - 1)%nat. (* .no-out *)
(*| .. coq:: none |*)
Proof.
...
Lemma GnawS_omega : forall i s, GnawS E0_omega (i::s)  = GnawS (E0finS i) s.
Proof.
...
Lemma gnaw_omega_n_SSn :
  forall n, gnaw T1omega (iota_from (S n) (S (S n))) = zero.
Proof. 
  intro n;  destruct n.  
  -  reflexivity. 
  -  apply gnaw_finite.
