...
(** To generalize ???? *)
  ...
  Definition rel_cast (r: Rel) : Relations L:= r.
  Coercion rel_cast  : Rel >-> Relations . 

  Definition fun_cast (f: Fun) : Functions L:= f.
  Coercion fun_cast  : Fun >-> Functions. 

(* begin snippet toyNotation *)
  (** Abreviations for the toy language L *)
  Notation A := (atomic _A Tnil).
  Notation B := (atomic _B Tnil). 
  Notation C := (atomic _C Tnil). 
  Notation P t := (atomic _P (Tcons t Tnil)).
  Notation R t1 t2 := (atomic _R (Tcons t1 (Tcons t2 Tnil))).

  Notation a := (apply _a Tnil).
  Notation b := (apply _b Tnil).
  Notation f t := (apply _f (Tcons t Tnil)).
  Notation g t := (apply _g (Tcons t Tnil)).

(* end snippet toyNotation *)

(* begin snippet smallTerms *)
(** a **)              
Goal apply _a Tnil = a%fol.
Proof. reflexivity. Qed. 

(** f a **)
Goal apply _f  (Tcons  (apply _a Tnil) Tnil) = (f a)%fol. 
Proof. reflexivity. Qed. 
