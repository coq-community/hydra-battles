...
Proof.
  ...
  - unfold freeVarTs in H; fold (freeVarT t) in H.
    ...
    unfold freeVarTs; simpl;
      fold (freeVarT (substT t v s));
      fold (freeVarTs (substTs ts v s));
      apply in_or_app.
    ...
    - intros v a H v0 s x H0 H1; rewrite subFormulaForall; 
      ...
      + induction (In_dec eq_nat_dec v (freeVarT s)) as [a0 | ?].
        * set (nv := newVar (v0 :: freeVarT s ++ freeVarF a)).
          ...
          --  intro H2; elim (newVar1 (v0 :: freeVarT s ++ freeVarF a)).
...
Proof.
  ...
  - intros f t0 H v s x H0 H1; simpl; unfold freeVarT;
      fold
        (freeVarTs (substTs  t0 v s)).
    apply H; auto.
  - intros v s x H H0; auto.
  - intros n t0 H t1 H0 v s x H1 H2; simpl; unfold freeVarTs in H2;
    fold (freeVarT t0) in H2; fold (freeVarTs t1) in H2.
    unfold freeVarTs; fold (freeVarT (substT t0 v s));
      fold (freeVarTs (substTs t1 v s));
      apply in_or_app.
...
Proof.
  intros H H0; induction ts as [| n t ts Hrects].
  - contradiction H0. 
  - simpl; unfold freeVarTs in H0; fold (freeVarT t) in H0.
    fold (freeVarTs ts) in H0; unfold freeVarTs.
    fold (freeVarT (substT t v s));
      fold (freeVarTs (substTs ts v s));
      apply in_or_app.
  ...
  - intros v a H v0 s x H0 H1; rewrite subFormulaForall.
    ...
    + induction (In_dec eq_nat_dec v (freeVarT s)) as [a0 | b0].
      * set (nv := newVar (v0 :: freeVarT s ++ freeVarF a)).
        simpl; apply in_in_remove.
        -- intro H2; 
             elim (newVar1 (v0 :: freeVarT s ++ freeVarF a)).
           fold nv; simpl; right.
           apply in_or_app.
           rewrite <- H2; auto.
        -- apply H.
...
Proof.
  ...
  - intros n t0 H t1 H0 v s x H1; simpl in H1;
      ...
      fold (freeVarT (substT t0 v s)) in H1;
      fold (freeVarTs (substTs t1 v s)) in H1.
    ...
    + induction (H0 _ _ _ H2) as [H3 | H3].
      * left; apply in_in_remove.
        ...
        -- unfold freeVarTs; fold (freeVarT t0);
             fold (freeVarTs t1); apply in_or_app.
...
Proof.
  ...
  - simpl in H; unfold freeVarTs in H;
      fold (freeVarT (substT t v s)) in H;
      fold (freeVarTs (substTs ts v s)) in H;
    ...
    + induction (freeVarSubTerm3 _ _ _ _ H0) as [H1 | H1].
      * left.
        ...
        -- unfold freeVarTs; fold (freeVarT t);
             fold (freeVarTs ts); apply in_or_app.
    ...
    + induction (Hrects H0) as [H1 | H1].
      * left; apply in_in_remove.
        -- eapply in_remove_neq; apply H1.
        -- unfold freeVarTs; fold (freeVarT t);
             fold (freeVarTs ts); apply in_or_app; right.
           eapply in_remove; apply H1.
      * now right.
Qed.
  ...
  - intros  v a H v0 s x H0; rewrite subFormulaForall in H0.
    ...
    + induction (In_dec eq_nat_dec v (freeVarT s)) as [a0 | ?].
      * set (nv := newVar (v0 :: freeVarT s ++ freeVarF a)).
        ...
        -- assert
            (H1: lt_depth L (substF a v (var nv))
                   (forallH v a)).
           { unfold lt_depth; rewrite subFormulaDepth.
             apply depthForall.
           }
           ...
           ++ assert (H4: lt_depth L a (forallH v a)) by
              apply depthForall.
              assert (H5: In x (freeVarF (substF a v (var nv)))).
              { eapply in_remove; apply H0. }
              assert (H6: x <> v0).
      ...
      * assert (H1: lt_depth L a (forallH v a)) 
        ...
        simpl in H0.
        assert (H2: In x (freeVarF (substF a v0 s))).
        { eapply in_remove, H0. }
...
Proof.
  ...
  - intros n t0 H t1 H0 v s x H1 H2; simpl in H1.
    ...
    fold (freeVarTs (substTs t1 v s)) in H1;
    unfold freeVarTs ;
      fold (freeVarT t0); fold (freeVarTs t1).
    ...
    + apply in_or_app; right; eapply H0.
      ...
      *  intro H4; elim H2.
         unfold freeVarTs; fold (freeVarT t0); 
           fold (freeVarTs t1); apply in_or_app; now right.
Qed.
...
Proof.
  ...
  - simpl in H; unfold freeVarTs in H;
    ...
    + apply in_or_app; left; eapply freeVarSubTerm4.
      ...
      * intro H2; elim H0.
        unfold freeVarTs; fold (freeVarT t);
          fold (freeVarTs ts); apply in_or_app; now left.
    + apply in_or_app; right; eapply Hrects. 
      * apply H1.
      * intro H2; elim H0; unfold freeVarTs;
          fold (freeVarT t); fold (freeVarTs ts).
    ...
    + induction (In_dec eq_nat_dec v (freeVarT s)) as [a0 | ?].
      * set (nv := newVar (v0 :: freeVarT s ++ freeVarF a)).
        ...
        assert
          (H2: In x
             (freeVarF
                (substF (substF a v (var nv)) v0 s))).
        { eapply in_remove; apply H0. } 
        assert (H3: In x (freeVarF (substF a v (var nv)))).
        { eapply H.
          - unfold lt_depth; rewrite subFormulaDepth.
            apply depthForall.
          - apply H2.
...
Remark subFormulaNTEHelp (f g : Formula) (v : nat) (s : Term):
 ...
 SysPrf (Ensembles.Add _ (Empty_set Formula) (substF f v s))
   (substF g v s).
Proof.
 intro H; apply (impE L) with (substF f v s).
 - apply (sysWeaken L).
  ...
  - intros v0 s H0; 
    ...
    + eapply (sysExtend L) with (Empty_set Formula).
      ...
      * assert
        ...
        assert (H6: In v (freeVarF (substF a v (var x)))).
      ...
      * assert (H4: lt_depth L a (forallH v a))
        ...
        intros H5 H7 H8;
        set (A1 := substF a v (var x)).
        rewrite <- (subFormulaId _ a v).
        apply impE with
          (substF (substF a v (var x)) x (var v)).
        ...
        -- fold A1;
             apply impE with
             (substF (substF A1 x (var v)) v0 s).
           ++ apply sysWeaken,  (iffE1 L).
              assert 
                (H6: lt_depth L (substF A1 x (var v)) (forallH v a)).
      ...
      * apply (impI L).
        ...
        -- intros [x0 [H4 H5]];
             induction H5 as [x0 H5| x0 H5]; [ induction H5 | induction H5 ].
           auto.
        -- apply impE with (substF a v (var x)).
           ++ apply (iffE2 L).
              apply sysWeaken.
              assert
                (H4: lt_depth L (substF a v (var x)) 
                       (forallH v a)).
              { unfold lt_depth; rewrite subFormulaDepth; 
                  apply depthForall.
              }
                 ...
                 --- unfold not in |- *; intros.
                     ...
                     assert
                       (H11: In v
                               (freeVarF
                                  (substF 
                                     (substF a v (var x1)) v2 s)))
                     by (eapply in_remove, H10). 
                     assert (H12: In v (freeVarF 
                                     (substF a v (var x1)))).
                     ...
                     +++ induction H13 as [H13| H13].
                         ...
                         *** contradiction.
                 --- set (A1 := substF a v (var x1));
                       rewrite <- (subFormulaId L a v).
                     apply impE with (substF A1 x1 (var v)).
                     +++ apply sysWeaken.
                         apply (iffE1 L).
        ...
        -- decompose record
           ...
           ++ apply (iffI L).
              ...
              ** set (A1 := substF 
                      ...
                      substF 
                        (substF 
                           (substF  
                              ...
                              v1 (var v2)) 
                           x0 (var x1)) v2 s).
                 set (x2 := newVar (v1 :: v2 :: freeVarF A1 ++ 
                                      freeVarF A2)).
                 assert
                   (x2prop : ~ In x2 (v1 :: v2 :: 
                                        freeVarF A1 ++ 
                 ...
                 unfold In in x2prop.
                 fold (In x2 (freeVarF A1 ++ 
                                freeVarF A2)) in x2prop.
                 apply impE with
                   (substF 
                      (substF 
                         (substF 
                 ...
                 --- apply sysWeaken.
                     ...
                     +++ apply subFormulaNTEHelp; unfold A1;
                         ...
                         *** apply impE with
                             ...
                             apply (impI L).
                             apply subFormulaNTEHelp.
                             apply (impE L) with (substF a v (var x2)).
                             apply (sysWeaken L).
                             apply (iffE2 L).
              ...
              ** apply (impI L), forallI. 
                 ...
                 --- set
                     ...
                     assert
                       (x2prop : 
                         ~ In x2 (v1 :: v2 :: freeVarF A1 ++ 
                                    freeVarF A2)) by
                       ( unfold x2; apply newVar1).
                     unfold In in x2prop;
                       fold (In x2 (freeVarF A1 ++ freeVarF A2)) 
                       in x2prop.
                     apply impE with
                       (substF 
                          (substF 
  ...
  - intros v1 v2 s1 s2 H0 H1 H2; 
      assert
        (H3: forall (v1 v2 : nat) (s1 s2 : Term),
            ...
            SysPrf T
              (impH
                 (substF  (substF  (forallH v a) v1 s1)
                    v2 s2)
                 (substF  (substF  (forallH v a) v2 s2)
                    v1 s1))).
          ...
          * clear H7.
            ...
            -- apply (impI L).
               ...
               ++  intros [x3 [H15 H16]];
                   ...
                   ** rewrite a1 in H18.
                      induction H18 as [H18| H18]; auto.
               ++ apply impE with (substF  a v (var x0)).
                  ** apply sysWeaken.
                     apply (iffE2 L).
                     assert
                       (H15: lt_depth L (substF  a v (var x0)) 
                               (forallH v a)).
                     { unfold lt_depth in |- *.
                       repeat rewrite subFormulaDepth.
                       apply depthForall. }
                  ...
                  ** rewrite <- (subFormulaId L 
                     ...
                     --- apply impE with
                           (substF  
                              (substF  a v (var x)) x (var x0)).
                         +++ apply sysWeaken.
                             apply (impI L).
                             apply subFormulaNTEHelp.
                             apply (impE L) with (substF  a v (var x)).
                             *** apply (sysWeaken L).
                                 apply (iffE2 L).
            ...
            -- apply (impI L).
               ...
               ++ apply impE with
                    (substF 
                       (substF  (substF  a v (var x0)) x0
                          (var x2)) v1 s1).
                  ** apply sysWeaken.
                     apply (impI L).
                     repeat apply subFormulaNTEHelp.
                     apply (impE L) with (substF  a v (var x0)).
                     --- apply (sysWeaken L).
                         apply (iffE2 L).
                         assert
                           (H15: lt_depth L (substF  a v (var x0))
                             ...
                             (A1 :=
                                forallH z1
                                  (substF 
                                     (substF 
                                        (substF 
                                           (substF  (substF  a v (var x0)) v2
                                              s2) x0 (var x2)) v1 s1) x2 (var z1))). 
                             rewrite <-
                               (subFormulaId L
                                  (substF 
                                     (substF 
                                        (substF  (substF  a v (var x0)) v2 s2)
                                        x0 (var x2)) v1 s1) x2).
                             apply
                               impE
                               with
