...
Proof.
  change (Succ a <> Zero)%nn with
    (substituteFormula LNN (Succ(v#0) <> Zero)%nn 0 a).
...
Proof.
  change  (a + Zero = a)%nn with
    (substituteFormula LNN (v#0 + Zero = v#0)%nn 0 a).
...
Proof.
  change (a * Zero = Zero)%nn with
    (substituteFormula LNN (v#0 * Zero = Zero)%nn 0 a).
  - apply forallE, Axm; repeat (try right; constructor) || left.
Qed.
...
Proof.
  change (~ (a <Zero))%nn with
    (substituteFormula LNN (~ v#0 < Zero)%nn 0 a).
  - apply forallE, Axm; repeat (try right; constructor) || left.
Qed.
