(******************************************************************************)
(** @TODO : remove too strong indentations *)
...
(* Import LNN_notations. Compute betaFormula.
Locate andH. 
*)

Section ProofofBetaRepresentable.
...
Proof.
 ...
 { intros a b; unfold cPair;
   assert (H: 2 * sumToN (a + b) = (a + b) * S (a + b)) by
     (induction (a + b); simpl; lia).
    lia. }
 unfold Representable; split.
 -  intros v H; simpl in H; lia.
 - simpl; intros a a0; unfold betaFormula.
   assert
   ...
   { intros A v x a1 H; rewrite (subFormulaExist LNN).
    ...
    { intros t1 t2 v a1; unfold LT at 1; now rewrite (subFormulaRelation LNN). }
    repeat first
                        ...
                        ---- fold (var 4) in |- *.
                             ...
                             { apply impI. eapply eqTrans.
                               ...
                               +  apply eqPlus. eapply eqTrans.
                                 * apply sysWeaken; apply eqSym, natTimes.
                                 * eapply eqTrans.
                                   -- apply eqTimes.
                        ...
                        --- apply impE with (LT (var 0) 
                            ...
                            *** rewrite <-
                                               ...
                                               (existH 5
                                                  (andH (LT (var 5) (Succ (var 4)))
                                                     (equal
                                                        ...
                                                        (var 4)))) 
                                               (equal (var 0) (natToTerm (beta a a0)))))
                                         3 (natToTerm (cPairPi1 a))) 4 
                                      (natToTerm (cPairPi2 a))).
                                 
                                 ---- apply (subWithEquals LNN). 
                                      apply eqSym. eapply andE2.
                                 apply Axm; right; constructor.
                                 ----  do 2 apply sysWeaken.
                                 repeat first
                                   [ rewrite H; [| discriminate ]
                                 ...
                                 repeat (rewrite (subTermNil LNN);
                                         [| apply closedNatToTerm ]).
                                 repeat (rewrite (subTermNil LNN (natToTerm a0));
                                         [| apply closedNatToTerm ]).
                                 repeat (rewrite (subTermNil LNN 
                                                    (natToTerm (beta a a0)));
                                         [| apply closedNatToTerm ]).
                                 apply impTrans with 
                                   (LT (var 0) (natToTerm (S (cPairPi1 a * S a0)))).
                                 { apply impI.
                                   apply
                                     impE
                                     with
                                   ...
                                   - apply
                                       ...
                                       with
                                       (substituteFormula LNN (LT (var 0) (var 1)) 1
                                          (Succ (Times (natToTerm (cPairPi1 a)) 
                                                   (Succ (natToTerm a0))))).
                                     + apply (subWithEquals LNN). 
                                       apply sysWeaken. simpl in |- *.
                                     apply eqSucc.
      ...
      -  apply impI. unfold beta in |- *.
        ...
        + apply sysWeaken. rewrite H0. simpl in |- *. rewrite subTermNil.
           * replace (apply LNN Languages.Succ (Tcons LNN 0 (natToTerm a) (Tnil LNN))) 
               with (natToTerm (S a)) by reflexivity.
              apply natLT. apply Nat.lt_succ_r. 
        ...
        + rewrite H; try lia.
           *  apply existI with (natToTerm (cPairPi2 a)). 
              repeat rewrite (subFormulaAnd LNN). apply andI.
              --  apply sysWeaken. repeat rewrite H0. simpl in |- *.
                 repeat rewrite (subTermNil LNN (natToTerm a)).
                 ++ replace (apply LNN Languages.Succ (Tcons LNN 0 (natToTerm a) (Tnil LNN))) with (natToTerm (S a)) by reflexivity.
                 ...
                 ++ apply andI.
                     ...
                     ** repeat (rewrite H; [| discriminate ]).
                         ...
                         --- apply sysWeaken.
                             ...
                             apply andI.
                             +++ replace (apply LNN Languages.Succ (Tcons LNN 0 (natToTerm (cPairPi2 a)) (Tnil LNN))) with (natToTerm (S (cPairPi2 a))) by reflexivity.
...
Proof.
  ...
  - apply impE with A.
    + apply sysWeaken, H.
    + apply Axm; right; constructor.
  - simpl; apply existSys.
    + apply closedNN.
    + simpl; intro H0; pose proof (In_list_remove2 _ _ _ _ _ H0); congruence.
    + apply existSimp, Hrecn.
Qed.
...
Proof.
  intros n m A B H; induction n as [| n Hrecn]; simpl; [assumption|]. 
  apply (reduceExist LNN); [apply closedNN | assumption].
Qed.
...
Proof.
  intros n m A v s H H0. induction n as [| n Hrecn]; simpl; [trivial |].
  rewrite (subFormulaExist LNN); destruct (eq_nat_dec (n + m) v) as [e | e]; try lia.
...
Proof.
  ...
  apply Hrecn; eapply In_list_remove1, H.
Qed.
...
Proof.
  intros n m v A H; induction n as [| n Hrecn]; try lia.
  simpl in H |- *.
  assert (H0: In v (freeVarFormula LNN (addForalls m n A))) by
...
Proof.
  intros n m A v s H H0; induction n as [| n Hrecn]; simpl; [reflexivity| ].
...
Proof.
  unfold Representable; split.
  - simpl; lia.
  - intros a; unfold succFormula; rewrite (subFormulaEqual LNN); apply iffRefl.
Qed.
...
Proof.
  ...
  - induction n as [| n Hrecn]; try lia.
    simpl; intros a; destruct (Nat.eq_dec m n) as [e | e].
    + rewrite e; clear e Hrecn pr m; induction n as [| n Hrecn].
      * simpl; unfold projFormula; rewrite (subFormulaEqual LNN); apply iffRefl.
      * simpl; intros a0; unfold projFormula; repeat rewrite (subFormulaEqual LNN); simpl.
    ...
    + apply RepresentableAlternate with (equal (var 0) (var (S m))).
      * apply iffSym; apply (subFormulaNil LNN); simpl; lia.
      * auto.
Qed.
...
Proof.
  ...
  { intro n; induction n as [| n Hrecn]; simpl.
    - intros w m H v.
      induction v as [| a n v Hrecv]; simpl; intros B g H0 H1 H2.
      + unfold composeSigmaFormula; simpl.
        replace
         (subAllFormula LNN B
                       ...
                       --- simpl in H3. destruct (in_app_or _ _ _ H3) as [H5 | H5].
                           *** simpl in H4.
                             ...
                             elim (proj1 (Nat.le_ngt n0 0)).
                               ++++ decompose record H4 /r. 
                                    intros H7 H8; apply H7.
        ...
        * rewrite subAddExistsNice.
          -- apply reduceAddExists. rewrite (subFormulaAnd LNN).
             ...
             ++ apply iffSym. apply (subFormulaNil LNN).
                intro H4; 
                  decompose record (freeVarSubAllFormula1 _ _ _ _ H4) /r.
                intros  H4 x H6 H7. 
                destruct x as [| n0].
    ...
    + assert (m + S w <= v \/ v < S w).
      ...
      * simpl in H6. destruct (in_app_or _ _ _ H6).
        ...
        -- apply HrecA; auto.
           ++ simpl in H2. tauto.
           ++ lia.
    + decompose record (freeVarSubAllFormula1 _ _ _ _ H6) /r.
      intros x H8 H9;  destruct x as [| n0].
...
Proof.
  ...
  { intros P H d; induction d as [| d Hrecd].
    ...
    - intros c H0 H1; assert (c < d \/ c = d) as H2 by lia; destruct H2.
      + eauto.
      + destruct (boundedCheck P H c) as [H3 | H3].
        * exists c; tauto.
...
Proof.
  ...
  unfold minimize; apply impI.
  apply impE with
    (substituteFormula LNN
       (impH (LT (var x) (var v)) (notH (substituteFormula LNN B v (var x))))
       x (natToTerm a)).
  - rewrite (subFormulaImp LNN),  (subFormulaNot LNN).
    apply impTrans with
        (impH (LT (natToTerm a) (var v))
           (notH (substituteFormula LNN B v (natToTerm a)))).
    + apply iffE1, (reduceImp LNN).
      * unfold LT at 2; rewrite (subFormulaRelation LNN).
        simpl; destruct (eq_nat_dec x x) as [e | e]; try lia.
        destruct (eq_nat_dec x v).
        -- elim H; auto.
        -- apply iffRefl.
      * apply (reduceNot LNN), (subFormulaTrans LNN).
        intro H5; elim H0; eapply In_list_remove1; exact H5.
    + apply impTrans with (notH (LT (natToTerm a) (var v))).
      * apply impI; apply impE with 
          (notH (notH (substituteFormula LNN B v (natToTerm a)))).
        apply cp2.
        -- apply Axm; right; constructor.
      ...
      * apply impE with (notH (LT (var v) (natToTerm a))).
        ...
        -- repeat apply impI.
           ...
           ++ apply Axm; left; right; constructor.
        -- apply impI, orSys; repeat apply impI.
           ++ apply Axm; left; left; right; constructor.
        ...
        -- apply impE with (notH (notH A)).  
           apply cp2.
           ++ apply sysWeaken, boundedLT; intros n H5.
              rewrite (subFormulaNot LNN); auto.
           ++ apply nnI; eapply andE1; apply Axm; right; constructor.
  - apply forallE; eapply andE2, Axm; right; constructor.
  - apply impI; unfold minimize; 
      rewrite <-
        (subFormulaId LNN
           (andH A
              (forallH x
              ...
              ** apply impE with (LT (var x) (natToTerm a)).
                 --- apply sysWeaken; apply boundedLT; intros n H5.
                     ...
                     +++ apply cp2; apply iffE1; apply (subFormulaTrans LNN).
                         intro H6; elim H0; eapply In_list_remove1, H6.
                     +++ apply H4; auto.
                 --- apply Axm; right; constructor.
Qed.
    ...
    SysPrf NN
      (iffH (substituteFormula LNN (minimize A B v x) z s)
         (minimize (substituteFormula LNN A z s) (substituteFormula LNN B z s) v
            x)).
Proof.
  intros A B v x z s H H0 H1 H2; unfold minimize.
  rewrite (subFormulaAnd LNN), (subFormulaForall LNN).
  destruct (eq_nat_dec x z) as [e | e]; try congruence.
  destruct (In_dec eq_nat_dec x (freeVarTerm LNN s)) as [e0 | e0]; try tauto.
  rewrite (subFormulaImp LNN); unfold LT at 1.
  rewrite (subFormulaRelation LNN); simpl.
...
Proof.
  ...
  { induction n as [| n Hrecn].
    - simpl; intros A g H B h H0; unfold primRecSigmaFormula. 
      intros a; rewrite (subFormulaExist LNN).
      induction (In_dec eq_nat_dec 2 (freeVarTerm LNN (natToTerm a))) as [a0 | b].
      + elim (closedNatToTerm _ _ a0).
      + simpl; clear b; assert (repBeta : Representable 2 beta betaFormula) by
        apply betaRepresentable. 
        ...
        * apply (reduceExist LNN).
          ...
          -- apply (reduceAnd LNN).
             ...
             ++ apply iffRefl.
        * set (f := evalPrimRecFunc 0 g h). 
          destruct (betaTheorem1 (S a) f) as [[a0 b] p]; simpl in p.
          set (P := fun c : nat => forall z : nat, z < S a -> f z = beta c z).
          assert (H1: forall c : nat, decidable (P c)).
          { intros c; unfold decidable, P. set (Q := fun z : nat => f z <> beta c z).
            ...
            - left; unfold Q in H2; intros z H3; destruct (eq_nat_dec (f z) (beta c z)).
              + auto.
              + elim (H2 z); auto.
            - right; intro H3; destruct H2 as [x [H2 H4]]; elim H4. apply H3; auto. }
                ...
                ** apply (reduceAnd LNN).
                   --- assert
                       ...
                       { intros F a0 b c H1; rewrite (subFormulaExist LNN);
                         ...
                         - now elim H1. 
                         - destruct (In_dec eq_nat_dec b (freeVarTerm LNN (natToTerm a0))) 
                             as [H4 | H4].
                       ...
                       { intros F a0 b c H1; rewrite (subFormulaForall LNN);
                         ...
                         - now elim H1. 
                         - destruct 
                             (In_dec eq_nat_dec b (freeVarTerm LNN (natToTerm a0))) as [e0 | e0].
                           + elim (closedNatToTerm _ _ e0).
                           + reflexivity. }
                       apply minimize1.
                       +++ discriminate.
                           ...
                           *** apply existI with (natToTerm (f 0)). 
                               ...
                               ---- unfold f, evalPrimRecFunc. 
                                    ...
                                    ++++ apply iffE2,  (reduceSub LNN).
                                         **** apply closedNN.
                                         **** apply iffTrans with 
                                                (substituteFormula LNN A 2 (natToTerm x)).
                                              apply (reduceSub LNN).
                                              { apply closedNN. }
                                              { apply (subFormulaNil LNN);
                                                  intro H4; elim (proj1 (Nat.le_ngt 1 0)).
                                                - apply H; auto.
                                                - auto. }
                                    ...
                                    ++++ apply impE with
                                         ...
                                         **** rewrite (subFormulaEqual LNN); simpl. 
                                              ...
                                              { apply eqRefl. }
                                              { apply closedNatToTerm. }
                               ---- destruct repBeta as [H1 H4]; simpl in H4. 
                                    ...
                                    ++++ apply iffE2, (reduceSub LNN).
                                         ...
                                         **** apply (reduceSub LNN).
                                              ...
                                              { apply (subFormulaNil LNN); intro H5.
                                                destruct (freeVarSubFormula3 _ _ _ _ _ H5) 
                                                  as [H6 | H6].
                                                - now elim (In_list_remove2 _ _ _ _ _ H6). 
                                                - elim H6. }
                                    ++++ apply  impE  with
                                             (substituteFormula LNN (equal (var 0) 
                                                                       (natToTerm (beta x 0)))
                                                     ...
                                                     (substituteFormula LNN
                                                        (substituteFormula LNN
                                                           (substituteFormula LNN
                                                              (substituteFormula LNN
                                                                 (substituteFormula LNN 
                                                                    betaFormula 1 (var 3)) 2
                                                                 (natToTerm x)) 0 (var 1)) 3 
                                                           (natToTerm n)) 0
                                                        (natToTerm (f (S n)))) 1 
                                                     (natToTerm (f n))).
                                              { apply iffE1.
                                                            ...
                                                            (substituteFormula LNN
                                                               (substituteFormula LNN
                                                                  (substituteFormula LNN 
                                                                     betaFormula 1 (var 3)) 2
                                                                  (natToTerm x)) 3 
                                                               (natToTerm n)) 0 (var 1)) 1
                                                         (natToTerm (f n))).
                                                  + apply iffE2;
                                                      repeat (apply (reduceSub LNN); 
                                                              [ apply closedNN |]).
                                                    apply (subFormulaNil LNN). intro H6.
                                                    destruct (freeVarSubFormula3 _ _ _ _ _ H6)
                                                      as [H7 | H7].
                                                    ...
                                                    * apply impE with
                                                          ...
                                                          (H7: In 3
                                                             (freeVarFormula LNN 
                                                                (substituteFormula LNN 
                                                                   betaFormula 2 
                                                                   (natToTerm x)))) by
                                                         eapply In_list_remove1, H6.
                                                         destruct 
                                                           (freeVarSubFormula3 _ _ _ _ _ H7) 
                                                           as [H8 | H8].
                                                         ++ elim (proj1 (Nat.le_ngt 3 2)).
                                                            ** apply H4;
                                                                 eapply In_list_remove1, H8.
                                                            ** repeat constructor.
                                                      ...
                                                      -- apply
                                                         ...
                                                         ++ repeat 
                                                             ...
                                                             simpl.
                                                            repeat rewrite 
                                                              (subTermNil LNN (natToTerm 
                                                                                 (beta x n))).
                                                            ** rewrite H2. 
                                                               apply eqRefl. 
                                                               apply Nat.lt_lt_succ_r, H1.
                                                            ** apply closedNatToTerm.
                                                            ** apply closedNatToTerm. }
                                         **** apply andI.
                                                ...
                                                - apply impE with
                                                      (substituteFormula LNN
                                                         (substituteFormula LNN 
                                                            ...
                                                            (natToTerm (f (S n)))) 1
                                                         (natToTerm (f n))).
                                                  + apply iffE2; repeat (apply (reduceSub LNN);
                                                                         [ apply closedNN |]).
                                                    apply (subFormulaTrans LNN); intro H5; 
                                                      elim (proj1 (Nat.le_ngt 3 2)).
                                                    * apply H0; eapply In_list_remove1, H5.
                                                    * repeat constructor.
                                                  + apply
                                                     impE
                                                      with
                                                ...
                                                - apply impE with
                                                  ...
                                                  + apply iffE2; repeat (apply (reduceSub LNN);
                                                    ...
                                                    replace (natToTerm (S n)) with
                                                     (substituteTerm LNN (Succ (var 3)) 3 
                                                        (natToTerm n)).
                           ...
                           *** apply forallI.
                               ...
                               ---- destruct H as (H, H1). simpl in H1.
                                    apply
                                     impTrans
                                      with
                                        (substituteFormula LNN
                                           (substituteFormula LNN
                                              (equal (var 0) (natToTerm g)) 1 (natToTerm a))
                                           2 (natToTerm x)).
                                    ...
                                    ++++ repeat rewrite (subFormulaEqual LNN); simpl.
                                         repeat
                                          (rewrite (subTermNil LNN (natToTerm g));
                                           [| apply closedNatToTerm ]).
                                         apply impI.
                                         rewrite <-
                                          (subFormulaId LNN
                                             (substituteFormula LNN
                                                (substituteFormula LNN
                                                   (substituteFormula LNN
                                                      (substituteFormula LNN 
                                                         betaFormula 1 Zero) 2
                                                      (var 2)) 1 
                                                   (natToTerm a)) 2 (natToTerm x)) 0).
                                         apply impE with
                                             (substituteFormula LNN
                                                (substituteFormula LNN
                                                   (substituteFormula LNN
                                                      (substituteFormula LNN
                                                         (substituteFormula LNN betaFormula 
                                                            1 Zero) 2 
                                                         (var 2)) 1 (natToTerm a)) 
                                                   2 (natToTerm x)) 0 
                                                (natToTerm g)).
                                         ...
                                         **** apply sysWeaken; clear H1;
                                              ...
                                              { apply impE with
                                                    (substituteFormula LNN
                                                       (equal (var 0) (natToTerm (beta x 0))) 
                                                       0
                                                       (natToTerm (f 0))).
                                                - apply iffE2. apply (reduceSub LNN).
                                                  + apply closedNN.
                                                  ...
                                                  + apply sysWeaken;
                                                    ...
                                                    * destruct H0 as (H0, H4). simpl in H4.
                                                      ...
                                                      -- apply iffE1; 
                                                         ...
                                                         apply (subFormulaNil LNN); intro H5.
                                                         destruct 
                                                           (freeVarSubFormula3 _ _ _ _ _ H5)
                                                           as [H6 | [H6 | H6]].
                                                      ...
                                                      -- apply impTrans with
                                                         ...
                                                         ++ apply iffE1;
                                                            ...
                                                            elim (proj1 (Nat.le_ngt 3 2)).
                                                            ** apply H0. 
                                                               eapply In_list_remove1, H5.
                                                            ** repeat constructor.
                                                         ++ unfold f; simpl; apply iffE1, H4.
                                                    * rewrite <-
                                                       (subFormulaId LNN
                                                          (substituteFormula LNN
                                                             (substituteFormula LNN
                                                         ...
                                                         ++ apply impE with
                                                            ...
                                                            ** destruct repBeta as (H4, H5).
                                                               ...
                                                               --- apply iffE2. 
                                                                   ...
                                                                   intro H6.
                                                                   destruct 
                                                                     (freeVarSubFormula3 _ _ _
                                                                        _ _ H6) 
                                                                     as [H7 | H7].
                                                                   +++ elim 
                                                                       (proj1 (Nat.le_ngt 3 2)).
                                                                       *** apply H4. 
                                                                           eapply 
                                                                             In_list_remove1, H7.
                                                                       *** repeat constructor.
                                                                   +++ elim 
                                                                       (closedNatToTerm _ _ H7).
                                                               --- simpl in H5.
                                                                   apply
                                                                    impE
                                                                     with
                     ...
                     +++ intros b H1; 
                         ...
                         { unfold decidable. 
                           intros z; destruct (eq_nat_dec (f z) (beta b z)); auto. }
                         decompose record
                         ...
                         *** apply cp2. 
                             ...
                             ---- apply
                                   impE
                                    with
                                      (existH 0
                                         (fol.andH LNN
                                            (substituteFormula LNN 
                                               (substituteFormula LNN A 1 (natToTerm a)) 2
                                               (natToTerm b))
                                            (substituteFormula LNN
                                               (substituteFormula LNN
                                                  (substituteFormula LNN
                                                     (substituteFormula LNN betaFormula 1 Zero) 2
                                                     (var 2)) 1 (natToTerm a)) 2 
                                               (natToTerm b)))).
                                  ++++ apply sysWeaken. apply impI. apply existSys.
                                       **** apply closedNN.
                                       **** intro H1; destruct (in_app_or _ _ _ H1)
                                              as [H2 | H2].
                                            { elim (closedNatToTerm _ _ H2). }
                                            { elim (closedNatToTerm _ _ H2). }
                                       **** apply eqTrans with (var 0).
                                            { destruct H as (H, H1). simpl in H1. 
                                              apply eqSym. unfold f; simpl.
                                              apply impE with A.
                                              ...
                                              - apply
                                                  ...
                                                  with
                                                  (substituteFormula LNN 
                                                     (substituteFormula LNN A 1 
                                                        (natToTerm a)) 2
                                                     (natToTerm b)).
                                                + apply sysWeaken, iffE1.
                                                  apply iffTrans with 
                                                    (substituteFormula LNN A 2 (natToTerm b)).
                                                  * repeat (apply (reduceSub LNN);
                                                            [ apply closedNN |]).
                                                    apply (subFormulaNil LNN). 
                                                    intro H2. 
                                                    elim (proj1 (Nat.le_ngt 1 0)); auto.
                                                  * apply (subFormulaNil LNN);
                                                      intro H2; elim (proj1 (Nat.le_ngt 2 0)); 
                                                      auto.
                                                + eapply andE1. apply Axm; right; constructor. }
                                              ...
                                              - apply sysWeaken. apply iffE1.
                                                apply
                                                 iffTrans
                                                  with
                                                    (substituteFormula LNN
                                                       (substituteFormula LNN betaFormula 2 
                                                          (natToTerm b)) 1
                                                       (natToTerm 0)).
                                                + rewrite (subFormulaId LNN).
                                                  apply
                                                  ...
                                                  * repeat (apply (reduceSub LNN); 
                                                    ...
                                                    destruct (freeVarSubFormula3 _ _ _ _ _ H3)
                                                      as [H4 | H4].
                                  ...
                                  ++++ eapply andE1. apply Axm; right; constructor.
                             ---- apply impE with
                                    (equal (natToTerm (f x0)) (natToTerm (beta b x0))); 
                                    [| apply Hrecx0 ].
                                  ++++ clear Hrecx0.
                                       apply
                                        impE
                                            ...
                                            { apply impI.
                                              ...
                                              - apply sysWeaken. unfold LT. 
                                                repeat rewrite (subFormulaRelation LNN);
                                                  simpl.
                                                repeat 
                                                  (rewrite (subTermNil LNN (natToTerm a));
                                                   [| apply closedNatToTerm ]).
                                                fold (LT (natToTerm x0) (natToTerm a)). 
                                            ...
                                            { apply impI.
                                              assert
                                               (H1: forall n : nat,
                                                ...
                                                In n
                                                  (freeVarFormula LNN
                                                     (impH (equal (natToTerm (f x0))
                                                              (natToTerm (beta b x0)))
                                                        (equal (natToTerm (f (S x0))) 
                                                           (natToTerm (beta b (S x0))))))).
                                              { simpl; intros n H1.
                                                destruct (in_app_or _ _ _ H1) as [H2 | H2];
                                                  induction (in_app_or _ _ _ H2) as [H3 | H3];
                                                  elim (closedNatToTerm _ _ H3). }
                                              apply existSys.
                                              - apply closedNN.
                                              - apply H1.
                                              ...
                                              + apply closedNN.
                                              + apply H1.
                                              + unfold f at 2; simpl; fold (f x0).
                                                apply impE with
                                                  (equal (var 1) (natToTerm (beta b x0))).
                                                  * apply impI;
                                                      apply impE with 
                                                      (equal (var 0) (natToTerm 
                                                                        (h x0 (beta b x0)))).
                                                    -- repeat apply impI. 
                                                       apply eqTrans with (var 0).
                                                       ++ apply eqTrans with 
                                                            (natToTerm (h x0 (beta b x0))).
                                                          ** destruct (eq_nat_dec (f x0) 
                                                                         (beta b x0)) 
                                                               as [a0 | b0].
                                                                     ...
                                                                     *** simpl.
                                                                         ...
                                                                         ---- repeat 
                                                                             (apply 
                                                                              ...
                                                                              intro H4. 
                                                                              destruct 
                                                                                (freeVarSubFormula3 _ _ _ _ _ H4) 
                                                                                as [H5 | H5].
                                                                              ++++ 
                                                                                elim (proj1 (Nat.le_ngt 3 2)).
                                                                                **** 
                                                                         ...
                                                                         ---- apply iffRefl.
                                                             --- apply H3.
                                                          ** eapply andE2. eapply andE2. 
                                                             apply Axm; 
                                                               do 3 left; right; constructor.
                                                     -- apply impE with
                                                            (substituteFormula LNN
                                                               (substituteFormula LNN
                                                                  (substituteFormula LNN 
                                                  ...
                                                  * apply impE  with
                                                        (substituteFormula LNN
                                                           ...
                                                           (natToTerm x0)).
                                                    -- apply sysWeaken; apply iffE1. 
                                                       destruct repBeta as (H2, H3). simpl in H3.
                                                       apply
                                                        iffTrans
                                                         with
                                                           (substituteFormula LNN (equal (var 0) 
                     ...
                     +++ intros b H1. assert (H4: forall z : nat, decidable (f z = beta b z)).
                         ...
                         decompose record
                          (notBoundedForall (fun z : nat => f z = beta b z) (S a) H4 (H3 b H1)) /r.
                        intros x0 H6 H7;  apply impE with (notH (equal (natToTerm (f x0)) (natToTerm (beta b x0)))).
                                                ...
                                                (forallH 0
                                                   (forallH 1
                                                      (fol.impH LNN
                                                         ...
                                                         (fol.impH LNN
                                                            (substituteFormula LNN
                                                               (substituteFormula LNN (substituteFormula LNN B 2 
                                                                                         (var 3))
        ...
        -- unfold P. intros z H2. unfold beta. repeat rewrite cPairProjections1. 
        ...
        { apply Hrecn.
          ...
          - split.
            + intros v H3.
              ...
              * assert (H4: v <= S (S (S n))) by (apply H0; auto); lia.
              * elim (closedNatToTerm _ _ H4).
          ...
          -- apply
             ...
             ++ apply (reduceSub LNN). apply closedNN. unfold primRecSigmaFormula.
                ...
                { intros F a1 b c H5. 
                  ...
                  destruct (eq_nat_dec b c) as [e | e].
                  - elim H5. auto.
                  - destruct (In_dec eq_nat_dec b (freeVarTerm LNN (natToTerm a1))) 
                      as [e0 | e0].
                    + elim (closedNatToTerm _ _ e0).
                    + reflexivity. }
                ...
                { intros F a1 b c H6 H7. rewrite (subFormulaForall LNN). 
                  destruct (eq_nat_dec b c) as [e | e].
                  - elim H6. auto.
                  - destruct (In_dec eq_nat_dec b (freeVarTerm LNN (var a1))) as [e0 | e0].
                    + destruct e0 as [H8 | H8].
                ...
                { intros f a1 s. rewrite (subFormulaExist LNN). 
                  destruct (eq_nat_dec a1 a1) as [e | e].
                  - reflexivity.
                  - elim e; auto. }
                assert
...
Proof.
  ...
  - simpl in |- *; intros n0 m g H h H0. decompose record H /r.
    intros H1 H3 H4.
    assert
      (Representable n0
