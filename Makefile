...
COQLIBS?= -R ../../Eindhoven/Pocklington Pocklington\
  -R . Goedel
COQDOCLIBS?=-R ../../Eindhoven/Pocklington Pocklington\
  -R . Goedel
...
uninstall_me.sh:
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Goedel && rm -f $(VOFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Goedel" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
