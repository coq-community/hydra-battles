...
\hline
Name & Ascii&Math& Description& Where \\\hline
 \texttt{on\_lt} & \texttt{alpha o< beta}&$\alpha<\beta$& ordinal inequality & \vref{sect:on-lt-notation}\\
 \texttt{on\_le} & \texttt{alpha o<= beta}&$\alpha\leq\beta$& ordinal inequality & \vref{sect:on-lt-notation}\\
\texttt{oplus} & \texttt{alpha o+ beta} & $\alpha \oplus \beta$ & Hessenberg sum & \vref{sect:infix-oplus} \\
\texttt{round} & \texttt{h -1-> h'} & & round of a battle & \vref{sect:infix-round} \\
\hline
\end{tabular}
