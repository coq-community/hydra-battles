...
\texttt{ON} for representing countable ordinals, and a binary relation
 \texttt{lt}. Note that, in our development, \texttt{ON} is a type, while the \emph{set} of countable ordinals (called $\mathbb{O}$ by Schütte) 
is the full set over the type \texttt{ON}.

\index{Types!ON@ON (countable ordinals)}
...
(strictly) bounded if and only if it is countable. We use Florian Hatat's library on countable sets, written as he was a student of  \emph{\'Ecole Normale Supérieure de Lyon}.
...
According to the  definition of a well order, every non-empty subset of \texttt{ON} has a least element. Furthermore, this least element is unique.
...
Let us import the library \texttt{Coq.Logic.Epsilon}, which contains the following axiom.
...
\index{Functions!iota description operator}
...
In order to use these tools,  we had to tell \coq{}  that the type \texttt{ON} is not empty:
...
\begin{enumerate}
\item Prove that there exists a unique least member of \texttt{ON}
\item Prove that being a least member of \texttt{ON} entails the announced inequality 
\end{enumerate}
...
\linebreak \Verb|{exists! a: ON, least_member (Empty_set ON) a|, the only properties we would be able to prove about \texttt{bottom} would be \emph{trivial} properties, 
\emph{i.e.}, satisfied by \emph{any} element of type \texttt{ON}, like for instance
...
The definition of the function \texttt{succ:ON -> ON} is very concise. The successor of any ordinal $\alpha$ is the smallest ordinal strictly greater than $\alpha$.

\index{Functions!succ (in type ON)}
...
of the  (denumerable) range of any sequence \texttt{s: nat -> ON}. 
...
forall B : Ensemble ON,
 exists ! S : Ensemble ON, 
      exists f : ON -> ON, ordering_function f S B
\end{Coqanswer}


\begin{Coqanswer}
ordering_function_unicity :
forall (B S1 S2 : Ensemble ON) (f1 f2 : ON -> ON),
...
We are now ready to define and study addition on the type \texttt{ON}.
...
\index{Functions!phi0@phi0 (exponential of base omega)}
...
\section{An embedding of \texttt{T1} into \texttt{ON}}


Our library 
\href{../src/html/hydras.Schutte.Injection_from_T1.html}%
{\texttt{Schutte.Injection\_from\_T1.v}} establishes the link between two very different modelizations of ordinal numbers. In other words, it ``validates'' a data structure in terms of
a classical mathematical discourse considered as a model. 
First, we define a function from \texttt{T1} into  \texttt{ON} by structural recursion.
...
\texttt{members epsilon0} of the elements of \texttt{ON} strictly less than  $\epsilon_0$.
