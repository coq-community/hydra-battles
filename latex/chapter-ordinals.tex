\chapter{Introduction to ordinal numbers}
...
\subsection{Examples of Notation systems}
...
\vspace{4pt}
\noindent\emph{From Library~\href{../src/html/hydras.OrdinalNotations/Omega_ON.html}{OrdinalNotations.Omega\_ON}}
...
In order to build an instance of \texttt{ON}, we have to verify that \texttt{lt} is a well-founded strict order and define a correct comparison function.
...
More complex examples will be developped in further sections or chapters. For instance,
a notation system for ordinals less than $\epsilon_0$ is presented in chapter~\ref{chap:T1}.




\subsection{Comparing Ordinal Notations}

It is sometimes useful to compare two ordinal notations with respect to expressive power
(the segment of ordinals one they represent). 

The following class specifies a strict inclusion of segments. The notation \texttt{OA} describes a segment $[0,alpha($, and \texttt{OB} is a larger segment (which contains a notation for $\alpha$). We require that the comparison functions of the two notation systems are compatible.
...
Finally, it may be interesting to compare an ordinal notation with the more theoretical model from Schütte (well, at least with our formalization of thhat model). This would be a relative proof of correctenss of our ordinal  notations.
...
For instance, the following theorem tells that \texttt{Epsilon0}, our otation system for the segment $[0,\epsilon0)$ is correct with respect to the countqable ordinal $\epsilon_0$
