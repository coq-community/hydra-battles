...
For instance, let us take $N_A=\texttt{Omega}$, and take $\alpha=(4,4,2,1,0)$,
 $\beta=(4,3,3,3,3,3,2)$, and $\gamma=(5)$. Then $\beta<\alpha<\gamma$. 

In contrast the list $(5,6,3,3)$ is not non-increasing (\emph{i.e.} sorted w.r.t. $\geq$), so it is not to be considered.
