...
we used an auxiliary function \texttt{canonS} of type \texttt{nat -> T1 -> T1} such that
\texttt{canonS i $\alpha$} is equal to $\canonseq{\alpha}{i+1}$.
...
\begin{Coqsrc}
Example canon3 :  canon 3 (omega ^ omega) = omega ^ 3.
...
Lemma canonS_succ i alpha :
  nf alpha ->  canonS i (succ alpha) = alpha.
Proof.
 induction alpha.
 (* ... *)
\end{Coqsrc}
...
Lemma canonS_LT i alpha :
  nf alpha -> alpha <> zero -> canonS i alpha <  alpha.
\end{Coqsrc}
...
Inductive standard_pathR(j:nat)( beta:T1):  nat -> T1 -> Prop :=
  std_1 : forall i alpha, 
       beta = canon i alpha -> j = S i ->
       standard_pathR j beta i  alpha
| std_S : forall i alpha, 
      standard_pathR j beta (S i) (canon i alpha)  ->
      standard_pathR j beta i alpha.
...
Definition const_pathS i :=
    clos_trans_1n T1 (fun alpha beta => beta = canonS i alpha).
