...
\subsection{A first theorem of impossibility}
...
We show that this is the case of the type \texttt{nat} of Peano natural numbers, provided with the usual strict order $<$. 

\subsubsection{Using the Hydra's height as a variant}
Trivially, a measure like the height will not work for building a variant.
...
\subsubsection{Conclusion/Teaser}

In order to build a variant for proving the termination of all hydra battles, we need to consider order structures more complex than the usual order on type \texttt{nat}. The next chapter will present the notion of \emph{ordinal notation} from the point view of the \coq{} user: inductive types, functions, type classes, etc. 
The ordinal $\epsilon_0$, the first one allowing us to build our desired variant, will be studied in detail in 
Chapter~\ref{chap:T1}.
