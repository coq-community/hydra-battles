...
Section Infinite_union.
  ...
  Section Indexed_union_lemmas_2.
    ...
    Let b_n (n : nat) :=
      epsilon (inhabits(Empty_set U)) (fun b => In A b /\ R b n).

    Remark bn_R_n :
      forall n, (exists b, In A b /\ R b n) -> R (b_n n) n.
    Proof.
      intros n ex_b.
      unfold b_n; apply epsilon_ind.
      auto.
      intros a (_, a_R_n); assumption.
    Qed.

    Let R_b (b : Ensemble U) :=
      epsilon (inhabits (fun _ _  => True)) (fun R => rel_numbers b R).

    Remark Rb_numbers_b :
      forall b, In A b -> rel_numbers b (R_b b).
    Proof.
      intros b b_In_A.
 unfold R_b; apply epsilon_ind.
      apply all_b_denum.
      assumption.
     auto.
Qed.
