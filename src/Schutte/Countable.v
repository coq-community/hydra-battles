...
(*** To do : merge into Countable :
             bulletize the proof scripts
             rename denumerable -> countable *)
...
Require Import MoreEpsilonIota PartialFun.

Require Import GRelations.
Require Import Arith_lemmas.
Import Lia.
...
Section Countable.
  ...
  A is countable). *)
    ...
    (** A is countable when there exists an injection from [A] to [Full_set nat]. *)
    ...
    Section Equivalence_with_surjection.
      ...
      Proof.
        ...
        unfold countable; intro denum.
        destruct denum as (R, R_enum).
        exists (rel_inv A Dnat R).
        red; apply R_inv_surj.
        assumption.

        intro ex_surj.
        elim ex_surj; intros R R_surj.
        exists (rel_inv Dnat A R).
        red.
        apply R_inv_inj.
        assumption.
      Qed.
  ...
  Section Countable_union.
    ...
    Section Countable_union_lemmas.
      ...
      Proof.
        ...
        inversion a'_R_b as
            [na' a'_In_E a'_RE_na' dna'_b | na' a'_In_F a'_RF_na' sdna'_b].
        ...
        inversion a'_R_b as
            [na' a'_In_E a'_RE_na' dna'_b | na' a'_In_F a'_RF_nb sdna'_b].
  ...
  Section Infinite_union.
    ...
    Section Infinite_union_lemmas.
      ...
      Definition K_1 : nat * nat -> nat := fun couple =>
                                             let (p, q) := couple in
                                             div2 ((p+q+1)*(p+q)) + q.

      Let K_rel (p1 : nat*nat) (p2 : nat*nat) : Prop :=
        let (p, q) := p1 in let (p', q') := p2 in
                            (p + q < p' + q') \/ (p + q = p' + q' /\ p' < p).

      Let f : nat * nat -> sigT (fun _ : nat => nat) := fun cpl =>
                                                          let (p, q) := cpl in
                                                          existT (fun _ : nat => nat) (p + q) q.

      Let R_K := (fun x y => (lexprod _ (fun _ => nat) lt (fun _ => lt)) (f x) (f y)).

      Lemma lexof_wf :
        well_founded R_K.
      Proof.
        unfold R_K.
        apply wf_inverse_image with (R := lexprod _ (fun _ => nat) lt (fun _ => lt)).
      ...
      Proof.
        ...
        case HK_rel.
        intros Hin.
        unfold R_K; unfold f; apply left_lex; assumption.
        intros (Hdiag, Hin).
        unfold R_K; unfold f; rewrite Hdiag; apply right_lex.
        case (le_lt_dec q' q).
        intros Hqin.
        case (lt_irrefl (p + q)).
      ...
      Proof.
        ...
        simpl K.
        replace (K (div2 ((p + 1) * p) + p)) with (0, p).
        trivial.
      ...
      Proof.
        ...
        case O_S with q'.
        replace 0 with (snd (S q, 0)); replace (S q') with (snd (r, S q')); try (compute; trivial; fail).
        apply (f_equal (snd (A := nat) (B := nat))); assumption.
        ...
        (* p = S r ; p' = S r' *)
        intros r' p'_eq r p_eq; rewrite p_eq in Heq; rewrite p'_eq in Heq.
        injection Heq; auto.
      Qed.
      ...
      Proof.
        ...
        cut (exists p, R b p).
        intros ex_p; elim ex_p; intros p b_R_p.
        cut (exists q, R_n p a q).
        intros ex_q; elim ex_q; intros q a_Rn_q.
        exists (K_1 (p, q)).
        red.
        replace (K (K_1 (p, q))) with (p, q).
        exists b.
      ...
      Proof.
        ...
        elim ex_b; intros b ((b_In_A, b_R_p), (b_In_a, a_Rnp_q)).
    ...
    Section Indexed_union_lemmas_2.
      ...
      Remark Rn_numbers_bn :
        forall n : nat, In A (b_n n) -> R (b_n n) n -> rel_numbers (b_n n) (R_n n).
      Proof.
        intros n bn_In_A.
        split.
        intros x x_In_b.
      ...
      Proof.
        ...
        pattern (b_p b).
        epsilon_elim.
        destruct R_enum as (R_dom, _, R_inj).
        elim (R_dom b b_In_A).
        intros p b_R_p.
        exists p.
        pattern (b_n p).
        epsilon_elim.      exists b; split; assumption.
      ...
      Lemma all_b_relation :
        ...
        forall n : nat, forall b : Ensemble U, In A b -> R b n -> rel_numbers b (R_n n).
      Proof.
        exists R_n.
        intros n b b_In_A b_R_n.
        assert (b = b_n n).
        pattern (b_n n).
        epsilon_elim.  
    ...
    Theorem countable_union_qcq :
      countable A ->
      (forall b : Ensemble U, In A b -> countable b) -> countable Infinite_union.
    Proof.
      intros A_denum all_b_denum.
      elim A_denum; intros R R_enum.
      elim all_b_relation with R; try assumption.
      intros R_n R_n_enum.
      exists (R_union_qcq R R_n).
