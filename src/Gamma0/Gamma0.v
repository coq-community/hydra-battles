...
Definition  phi (alpha beta : T2) : T2 :=
  match beta with zero => [alpha, beta] 
             ...
             | gcons b1 b2 0 (gcons zero zero  n zero) => 
               (match compare alpha b1
                with  Datatypes.Lt => 
                      [alpha, (gcons b1 b2 0 (finite n))]
                | _ =>  [alpha, (gcons b1 b2 0 (finite (S n)))]
                end)
...
Proof.
 ...
 rewrite (IHalpha1 beta1).
 rewrite (IHalpha2 beta2).
...
Lemma nf_nat_irrelevance : forall a b n n' c, nf (gcons a b n c) -> 
                                              nf (gcons a b n' c).
Proof.
...
generalize (phi_to_psi_1 beta2  0 H3).  
  simpl (succ (gcons beta1 beta2 0 zero)).
